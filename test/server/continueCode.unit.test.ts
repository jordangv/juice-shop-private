...
void describe('continueCode', () => {
  ...
  void it('should be signed hashid value of IDs of solved challenges', () => {
    ...
    continueCode()(req, res)
    assert.equal(res.json.mock.calls.length, 1)
    const code = res.json.mock.calls[0].arguments[0].continueCode
    assert.equal(utils.verifyContinueCode('continueCode', code), 'yXjv6Z5jWJnzD6a3YvmwPRXK7roAyzHDde2Og19yEN84plqxkMBbLVQrDeoY')
  })
})
